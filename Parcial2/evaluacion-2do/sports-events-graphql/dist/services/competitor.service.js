...
let CompetitorService = class CompetitorService {
    ...
    async createCompetitor(createCompetitorInput) {
        const existingCompetitor = this.persistence.findOneBy('competitors', {
            email: createCompetitorInput.email
        });
        if (existingCompetitor) {
            throw new common_1.ConflictException(`Ya existe un competidor con el email ${createCompetitorInput.email}`);
        }
        const competitor = new competitor_entity_1.Competitor(Object.assign(Object.assign({ id: (0, uuid_1.v4)() }, createCompetitorInput), { createdAt: new Date(), updatedAt: new Date() }));
    ...
    async updateCompetitor(id, updateCompetitorInput) {
        ...
        if (updateCompetitorInput.email && updateCompetitorInput.email !== existingCompetitor.email) {
            ...
            if (emailInUse && emailInUse.id !== id) {
                throw new common_1.ConflictException(`El email ${updateCompetitorInput.email} ya está en uso por otro competidor`);
            }
        }
        const updatedCompetitor = this.persistence.update('competitors', id, updateCompetitorInput);
